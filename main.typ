...
=== 跨平台性能对比
在高端与低端平台的弹性部署上，笔者没有做任何复杂的事情。有些高端特性（如GI和基于缓冲的屏幕空间后处理）原生不支持移动端，模拟时会直接剔除；资产流送参照虚幻引擎默认解决方案处理。唯一值得提及的是，由于资产本身没有任何阻止向下兼容的特性，直接将高端平台的低规格内容用于低端平台即可。降级细节见表4.2：
  #figure(
    table([1], [2]),
    caption: [降级细节],
    placement: none
...
==== 测试模块设计
设计了延迟测试类BP_LatencyLogger和接口BPI_LatencyTest。延迟测试类的功能是接收某次输入的绝对时间，并计算和统计与下一次输入被处理的绝对时间之差。使用Map记录不同操作产生的输入数据（下文样例中为Move），便于筛选和导出。任何实现了BPI_LatencyTest接口的Actor都可以被测试。操作时间将被输出到日志文件中。笔者在中间层IH_Move（承接虚幻引擎基建传来的原始数据）中实现该接口，这样测试数据仅针对中间件。重点比照不同输入手段两次处理之间的延迟。编写Python脚本对数据进行清晰、检验和对比：

使用Shapiro-Wilk检验评估数据正态性，使用Levene检验评估方差齐性。源日志见附录2，经过清理和检验的数据见表4.4，数据对比结果见表4.5。
...
可以看到，两种设备最典型的延迟表现（中位数）均为17.00ms，表示用户在多数情况下接受到的响应速度类似。键盘的平均延迟与手柄差值在统计上显著。
